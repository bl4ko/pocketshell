...
- **Terminal buffer NUL cells poison copied text**: TUIs skip blank cells with cursor moves, leaving code-0 cells that `translateToString` renders as `\u{0}`; remote shells silently drop pasted NULs (`export TART_HOME` → `exportTART_HOME`). Any text extracted from the buffer must scrub `\u{0}` → space — `SelectionText.join` (drag-selection copy) and `TerminalBridge.visibleText()` both do.
